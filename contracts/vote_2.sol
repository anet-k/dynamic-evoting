...
contract SubmitVote {
    ...
    function verifyVote(string memory choice, uint256 tokenId) public {
        //5b.1.1.1 smart contract check if the NFT token is valid, then proceed with 5b.1.2
    ...
    //submit altered vote
    function requestAlteration(string memory originalVote, uint256 tokenId) public {
        //check nft ownership
        require(nft.ownerOf(tokenId) == msg.sender, "Unauthorised access");
    ...
    function verifyAlteration(string memory alterationChoice, uint256 tokenId) public {

        require(nft.ownerOf(tokenId) == msg.sender, "Unauthorised access"); 

        require(blindedChoices[tokenId].blindedVote == keccak256(abi.encodePacked(alterationChoice)), "Invalid vote");
